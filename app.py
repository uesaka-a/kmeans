...
# ---------------------------
st.markdown(
    "<h3 style='font-size:28px;'>PCAによる2次元可視化</h3>",
    unsafe_allow_html=True
)
...
for cluster_id in range(k):
    ...
    plt.scatter(xs, ys, label=f"{cluster_id}", s=6)  # ← 点も少し小さく

plt.legend(fontsize=3)          # ← 小さく
plt.xlabel("PC1", fontsize=3)   # ← 小さく
plt.ylabel("PC2", fontsize=3)   # ← 小さく
plt.xticks(fontsize=2)          # ← 目盛り小さく
