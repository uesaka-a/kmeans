...
fig = plt.figure(figsize=(3, 2))  # ← 図自体も少し小さく

for cluster_id in range(k):
    xs = X_2d[labels == cluster_id, 0]
    ys = X_2d[labels == cluster_id, 1]
    plt.scatter(xs, ys, label=f"{cluster_id}", s=10)  # ← 点も少し小さく

plt.legend(fontsize=4)          # ← 小さく
plt.xlabel("PC1", fontsize=5)   # ← 小さく
plt.ylabel("PC2", fontsize=5)   # ← 小さく
plt.xticks(fontsize=4)          # ← 目盛り小さく
