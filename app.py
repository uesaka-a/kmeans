...
# ---------------------------
st.markdown(
    "<h3 style='font-size:22px;'>クラスタ</h3>",
    unsafe_allow_html=True
)
...
# ---------------------------
st.markdown(
    "<h3 style='font-size:22px;'>PCAによる2次元可視化</h3>",
    unsafe_allow_html=True
)

plt.rcParams.update({'font.size': 6})

fig = plt.figure(figsize=(3.2, 2.4))  # ← 図自体も少し小さく

for cluster_id in range(k):
    xs = X_2d[labels == cluster_id, 0]
    ys = X_2d[labels == cluster_id, 1]
    plt.scatter(xs, ys, label=f"{cluster_id}", s=20)  # ← 点も少し小さく

plt.legend(fontsize=6)          # ← 小さく
plt.xlabel("PC1", fontsize=7)   # ← 小さく
plt.ylabel("PC2", fontsize=7)   # ← 小さく
plt.xticks(fontsize=6)          # ← 目盛り小さく
